...
SfnRegistry.register(:all_rds_snapshots) do |filter = nil|
  if filter.nil?
    ...
    rds_snapshots.sort! { |a, b| b.snapshot_create_time <=> a.snapshot_create_time }.map(&:db_snapshot_identifier) << 'none'
  end
end
...
SfnRegistry.register(:latest_rds_snapshot) do |filter = nil|
  if filter.nil?
    ...
    last = rds_snapshots.sort! { |a, b| b.snapshot_create_time <=> a.snapshot_create_time }.map(&:db_snapshot_identifier).last
    last.nil? ? 'none' : last
  end
end
